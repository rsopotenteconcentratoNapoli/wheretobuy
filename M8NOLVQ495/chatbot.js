...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Digita la tua domanda...' +'"'+ '>' +
...
addChatbotMessage("Ciao! Sono il chatbot. Come posso aiutarla? (il primo messaggio può richiedere alcuni secondi)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_ita.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Recupero risposta...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Domanda:</strong> ' + question + '<br><strong>Answer:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Impossibile recuperare la risposta.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
